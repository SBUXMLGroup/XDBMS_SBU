...
import xmlProcessor.RG.RG07.RGGroupedCorrelatedPlansTuple;
import xmlProcessor.RG.RG07.CorrelatedPlans;
 ...
 *
 */
public class EvaluationTreeNode implements Runnable,EvaluationTreeNodeInput
{
...
//	private EvaluationTreeNode parent;	
	...
	private boolean internalFinished;
	...
	private Queue<TwigResult> outputQueue;
        ...
        //**************FOR PARALLELIZING:
        private BlockingQueue<TwigResult> paralTRQueue;
        //**************
...
//	private ArrayList<TwinArrayList<QTPNode, TwinObject<Integer, Integer>>> upperNodes; //upper nodes and their level and PCR which regards to it their dewey ids should be produced
	...
	public EvaluationTreeNode(ArrayList<TwinObject<QTPNode, Set<StructuralSummaryNode>>> qNodes, QTPNode mainNode, RGGroupedCorrelatedPlansTuple groupedCorrelatedPlansTuple)
	{
		...
		this.outputQueue = new LinkedList<TwigResult>();
               ...
               //this.paralTRQueue=new BlockingQueue<>();
                this.paralTRQueue=new BlockingQueue<>();
                //****
...
//                } catch (InterruptedException ex) {
//                    System.err.println("InterruptedException occured in thread EvaluationTreeNode while runing function internalNext()");
//                }
//            }
//            
//        }
...
//	}
	...
	public void open() throws DBException, IOException,InterruptedException
	{
		...
		{
			this.head = null;
			this.internalFinished = true;//????shuld thread start here?
                        this.finished = true;
                        this.paralTRQueue.enqueue(null);
                        if(!started) 
                           this.thread.start();
		...
		if (children.size() == 1 && NOTChildren.size() == 0 && children.getL1(0) instanceof SelfInput)
		{
			...
			else
			{
				...
				this.selfInputWrapper.next();
                        ...
                        this.thread.start();
                        LogManager.log(10,this.mainNode.toString()+" started!");
                        start(true);
...
//                    //(TwigResult)paralQueue.getHead();
            //return this.head;
             LogManager.log(9," point14");
            LogManager.log(9,this.getMainNode().toString()+" peek in getHead()");
            TwigResult Test=(TwigResult)paralTRQueue.peek();
            LogManager.log(9,"here:getHead");
            return Test;
        }
	private TwigResult getInternalHead()
	{
        ...
        public void internalNext() throws DBException, IOException, InterruptedException
	{
...
//		else if (this.opType == QTPNode.OpType.AND)
		if (this.opType == QTPNode.OpType.AND)
		{
			...
			while (this.outputQueue.isEmpty() && !isInternalFinished());
	...
	public void next() throws InterruptedException
        {
            logManager.LogManager.log(8,"EVNode:"+this.mainNode.toString()+"deques");
            paralTRQueue.dequeue();
            if(paralTRQueue.peek()==null)
            {
                LogManager.log(9,"Peek to check FInished!");    
                this.finished = true; 
            }
        }
...
//		}// if (children.size() == 0)
	...
	private boolean andBalance(TwinArrayList<EvaluationTreeNodeInput, ArrayList<TwigResult>> inputs) throws DBException, IOException, InterruptedException
	{
		for (int i = 0; i < inputs.size() - 1; )
		{
			...
			{
				//all till now are the same internalNext input should be checked
				i++;
			}
		}
...
//		}
	...
	private TwinArrayList<EvaluationTreeNodeInput, ArrayList<TwigResult>> orBalance(TwinArrayList<EvaluationTreeNodeInput, ArrayList<TwigResult>> inputs, TwinArrayList<EvaluationTreeNodeInput, ArrayList<TwigResult>> selected) throws DBException, InterruptedException
	{
		...
		int c = -1;
		for (int i = 0; i < inputs.size() - 1; )
		{       logManager.LogManager.log(9,"g to point4");
			c = inputs.getL1(i).getHead().getRes(mainNode).compareTo(inputs.getL1(i+1).getHead().getRes(mainNode));
			logManager.LogManager.log(9,"point4");
                        if (c < 0)
			{					
				//this.advance(inputs.getL1(i), inputs.getL2(i));
	...
	public boolean isInternalFinished()
	{
		return this.internalFinished;
            //return !paralTRQueue.hasMoreData();
	}

      	public boolean isFinished()
	{
		//return this.internalFinished;            
            return this.finished;//!paralTRQueue.hasMoreData();
	}
...
//	}
	...
	private boolean andExclude(TwigResult res) throws DBException, IOException, InterruptedException
	{
		...
		int c;
		logManager.LogManager.log(9,"g to point5");
		c = res.getRes(mainNode).compareTo(selected.getL1(0).getHead().getRes(mainNode));
		logManager.LogManager.log(9,"point5");
		while(c > 0)
		{
			for(int i = 0; i < selected.size(); ++i)
			{
			...
			selected.clear();
			selected = orBalance(NOTChildren, selected);
			logManager.LogManager.log(9,"g to point6");
			c = res.getRes(mainNode).compareTo(selected.getL1(0).getHead().getRes(mainNode));
                        logManager.LogManager.log(9,"point6");
		}
...
//			ArrayList<TwigResult> subSet;
			...
			while (bl && c >0)
			{
				...
				if (bl)
				{
                                        logManager.LogManager.log(9,"g to point7");
					c = res.getRes(mainNode).compareTo(balanced.getL1(0).getHead().getRes(mainNode));
                                        logManager.LogManager.log(9,"point7");
				}
//				else
//				{						
//					for (int i = 0; i < balanced.size(); i++)
...
//		}
	...
	{
		logManager.LogManager.log(9,"g to point8");
		TwigResult key = input.getHead();
                logManager.LogManager.log(9,"point8");
		subSet.clear();
//		try
//		{
                logManager.LogManager.log(9,"g to point9:");
                DeweyID d1=key.getRes(mainNode);
                DeweyID d2=input.getHead().getRes(mainNode);
                LogManager.log(9,"d2:"+d2.toString());
		while(!input.isFinished() && (key.getRes(mainNode).compareTo(input.getHead().getRes(mainNode)) == 0))
//		while(!input.isFinished() && (d1.compareTo(d2) == 0))
		{
                        logManager.LogManager.log(9,"point9 passed");
...
//////////////////////////////////////////////////////////////////////////
	...
	{		
        ...
        public void run() 
        {
            try {
                //this.open();
               // logManager.LogManager.log(8,this.mainNode.toString()+" : "+this.getInternalHead().toString());
                LogManager.log(10,this.mainNode.toString()+" starting");                    
                paralTRQueue.setProducer(this);
                paralTRQueue.enqueue(this.getInternalHead());
                while(true)
                {
                    if(this.getInternalHead()==null)
                    {   
                        
                        LogManager.log(8,"this:"+this.mainNode+" parallelQueue Size: "+paralTRQueue.size());
                        break;                   
                    }
                    this.internalNext();
                    paralTRQueue.enqueue(this.getInternalHead());
                ...
                Logger.getLogger(EvaluationTreeNode.class.getName()).log(Level.SEVERE, null, ex);
            } catch (IOException ex) {
                Logger.getLogger(EvaluationTreeNode.class.getName()).log(Level.SEVERE, null, ex);
            } catch (InterruptedException ex) {
                Logger.getLogger(EvaluationTreeNode.class.getName()).log(Level.SEVERE, null, ex);
