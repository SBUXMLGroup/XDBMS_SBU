...
import xmlProcessor.RG.RGX.EvaluationTree.EvaluationTree;
import xmlProcessor.RG.RGX.EvaluationTree.EvaluationTreeNode;
import xmlProcessor.RG.RGX.EvaluationTree.EvaluationTreeNodeInput;
import xmlProcessor.RG.RGX.EvaluationTree.SelfInput;
...
import xmlProcessor.RG.RGX.SharedInput;
...
//		this.transaction = context.getTransaction();
    ...
    public RGResultSet execute() throws DBException, IOException, InterruptedException
	{	
		...
		for (int i = 0; i < correlatedPlans.size(); ++i)
		{
			...
			for (Entry<QTPNode, StructuralSummaryNode> entry : mapEntry)
			{
				...
				if (!targetPCRs.containsKey(node))
				{
					...
					targetPCRs.put(node, set);
				}
				else
				{
					HashSet<Integer> value = targetPCRs.get(node);
		...
		ArrayList<RGGroupedCorrelatedPlansTuple> groupedCorrelatedPlansTuples = this.groupCorrelatedExecutionPlans(groupedEqualPlans, targetPCRs);
    ...
    {    	
    	...
    	{
    		qNodes.add(new TwinObject<QTPNode, Set<StructuralSummaryNode>>(root.getParent(), correlatedPlan.getTargetCIDs(root.getParent())));
    	}
    	if (!root.hasChildren())
    	{
    		...
    		evaluationTreeRootNode.addChild(leaf, false);
                //what if we start children here?
               // new Thread(leaf).start();
...
//                                leaf.start(tL);
                               ...
                               // new Thread(evaluationTreeRootNode).start();
    			}
...
//    	groups.add(group);
    	...
    	while(correlatedPlans.size() > 0)
    	{
    		...
    		{
    			boolean found = false;
    			if (correlatedPlans.get(i).getTargetCIDs(sortedLeafNodes[leafNo]) != null)
    			{
...
//    	return groupedExecutionTuples;
    ...
    {
    	HashSet<StructuralSummaryNode>  ps2 = g2.getTargetCIDs(sortedLeafNodes[leafNo]);
    	for (StructuralSummaryNode psNode : ps2)
		{
        	for (int i = 0; i < g1.size(); i++)
    		{
        		HashSet<StructuralSummaryNode>  ps1 = g1.get(i).getTargetCIDs(sortedLeafNodes[leafNo]);
...
//	}
//	end of comment
	protected TwigResult next() throws DBException, IOException, InterruptedException
	{
...
//	long cmptime = 0;
	private TwigResult nextOrdered() throws DBException, IOException, InterruptedException
	{
